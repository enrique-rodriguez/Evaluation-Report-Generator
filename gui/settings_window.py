...
class SettingsWindow(Toplevel):
    ...
    def __init__(self, master, settings):
        ...
        self.max_points_label = Label(
            self.max_points_frame, text='Calificación Máxima por pregunta')

        self.max_points = IntVar(
            self.max_points_frame, value=settings["max_points_per_question"])
        self.max_points_per_question_spinbox = Spinbox(
            self.max_points_frame, textvariable=self.max_points, from_=1, to=100000)
        ...
        self.max_points_per_question_spinbox.pack(
            pady=PADDING, fill=X, side=LEFT)
        self.save_settings_button.pack(pady=PADDING, fill=X)

    def save_settings(self):
        self.settings["max_points_per_question"] = self.max_points.get()
        self.settings.save()
        messagebox.showinfo("Ajustes Guardados",
                            "Los ajustes han side guardados exitosamente")
        self.close()
